...
class AddGenerator<TEntity> : AbstractGenerator, IAddGenerator<TEntity> where TEntity : class
{
    ...
    public ExecuteUpdate<TEntity> CreateExecuteAddFunc()
    {
        ...
        var endIndex = il.DeclareLocal(typeof(int));
        // using(var reader = command.ExecuteReader())
        {
            ...
            il.BeginExceptionBlock();
            {
                ...
                // var endIndex = reader.GetInt32(0)
                il.EmitLdloc(reader);
                il.EmitLdc_I4(0);
                il.EmitCall(DbDataReader_GetInt32);
                il.EmitStloc(endIndex);
            }
            // finally
            {
                // reader?.Dispose()
        ...
        // for(var i = 0 ; i < entities.Length; i++)
        {
            ...
            il.Emit(OpCodes.Br, forEnd);
            {
                ...
                // entities[i].Id = endIndex - (entities.Length - 1 - i);
                il.EmitLdarga_S(1);
                il.EmitLdloc(i);
                il.EmitCall(Cast<TEntity>.Span_get_Item);
                il.Emit(OpCodes.Ldind_Ref);
                il.EmitLdloc(endIndex);
                il.EmitLdarga_S(1);
                il.EmitCall(Cast<TEntity>.Span_get_Length);
                il.Emit(OpCodes.Sub);
                il.EmitLdc_I4(1);
