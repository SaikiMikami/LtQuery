...
public class SqlBuilderTests
{
    ...
    public void CreateSelectSql_WithTake()
    {
        ...
        Assert.Equal("SELECT t0.`Id`, t0.`Title`, t0.`CategoryId`, t0.`UserId`, t0.`DateTime`, t0.`Content` FROM `Blog` AS t0 ORDER BY t0.`CategoryId` LIMIT 10", actual);
    }
    ...
    public void CreateSelectSql_WithSkipAndTake()
    {
        ...
        Assert.Equal("SELECT t0.`Id`, t0.`Title`, t0.`CategoryId`, t0.`UserId`, t0.`DateTime`, t0.`Content` FROM `Blog` AS t0 ORDER BY t0.`CategoryId` LIMIT 10, 5", actual);
    }
    ...
    public void CreateSelectSql_WithIncludeChildrenAndTake()
    {
        ...
        Assert.Equal("SELECT t0.`Id`, t0.`Title`, t0.`CategoryId`, t0.`UserId`, t0.`DateTime`, t0.`Content` FROM `Blog` AS t0 LIMIT @Take; SELECT t0.`Id`, t1.`Id`, t1.`BlogId`, t1.`UserId`, t1.`DateTime`, t1.`Content` FROM (SELECT t0.`Id` FROM `Blog` AS t0 LIMIT @Take) AS t0 INNER JOIN `Post` AS t1 ON t0.`Id` = t1.`BlogId`", actual);
    }
    ...
    public void CreateSelectSql_WithIncludeChildrenAndTakeAndSkip()
    {
        ...
        Assert.Equal("SELECT t0.`Id`, t0.`Title`, t0.`CategoryId`, t0.`UserId`, t0.`DateTime`, t0.`Content` FROM `Blog` AS t0 ORDER BY t0.`Title` LIMIT @Skip, @Take; SELECT t0.`Id`, t1.`Id`, t1.`BlogId`, t1.`UserId`, t1.`DateTime`, t1.`Content` FROM (SELECT t0.`Id` FROM `Blog` AS t0 ORDER BY t0.`Title` LIMIT @Skip, @Take) AS t0 INNER JOIN `Post` AS t1 ON t0.`Id` = t1.`BlogId`", actual);
    }
    ...
    public void CreateSelectSql_WithIncludeChildrenAndWhereAndTake()
    {
        ...
        Assert.Equal("SELECT t0.`Id`, t0.`Title`, t0.`CategoryId`, t0.`UserId`, t0.`DateTime`, t0.`Content` FROM `Blog` AS t0 WHERE t0.`DateTime` >= @DateTime LIMIT @Take; SELECT t0.`Id`, t1.`Id`, t1.`BlogId`, t1.`UserId`, t1.`DateTime`, t1.`Content` FROM (SELECT t0.`Id` FROM `Blog` AS t0 WHERE t0.`DateTime` >= @DateTime LIMIT @Take) AS t0 INNER JOIN `Post` AS t1 ON t0.`Id` = t1.`BlogId`", actual);
    }
    ...
    public void CreateSelectSql_WithIncludeChildrenAndComplexInclude()
    {
        ...
        Assert.Equal("SELECT DISTINCT t0.`Id`, t0.`Title`, t0.`CategoryId`, t0.`UserId`, t0.`DateTime`, t0.`Content`, t1.`Id`, t1.`Name`, t1.`Email`, t1.`AccountId` FROM `Blog` AS t0 INNER JOIN `User` AS t1 ON t0.`UserId` = t1.`Id` INNER JOIN `Post` AS t2 ON t0.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t0.`CategoryId` >= 4 AND t3.`Name` = @UserName LIMIT @Take; SELECT t0.`Id`, t2.`Id`, t2.`BlogId`, t2.`UserId`, t2.`DateTime`, t2.`Content`, t3.`Id`, t3.`Name`, t3.`Email`, t3.`AccountId` FROM (SELECT DISTINCT t0.`Id` FROM `Blog` AS t0 INNER JOIN `User` AS t1 ON t0.`UserId` = t1.`Id` INNER JOIN `Post` AS t2 ON t0.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t0.`CategoryId` >= 4 AND t3.`Name` = @UserName LIMIT @Take) AS t0 INNER JOIN `Post` AS t2 ON t0.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id`", actual);
    }
    ...
    {
        var query = Lt.Query<Category>().Include<User>(new[] { "Blogs", "Posts", "User" }).Where(_ => _.Blogs.Any(_ => _.DateTime < Lt.Arg<DateTime>("DateTime") && _.Posts.Any(_ => _.User!.Name == Lt.Arg<string>("UserName")))).Take("Take").ToImmutable();
        var actual = _inst.CreateSelectSql(query);

        Assert.Equal("SELECT DISTINCT t0.`Id`, t0.`Name` FROM `Category` AS t0 INNER JOIN `Blog` AS t1 ON t0.`Id` = t1.`CategoryId` INNER JOIN `Post` AS t2 ON t1.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t1.`DateTime` < @DateTime AND t3.`Name` = @UserName LIMIT @Take; SELECT t0.`Id`, t1.`Id`, t1.`Title`, t1.`CategoryId`, t1.`UserId`, t1.`DateTime`, t1.`Content` FROM (SELECT DISTINCT t0.`Id` FROM `Category` AS t0 INNER JOIN `Blog` AS t1 ON t0.`Id` = t1.`CategoryId` INNER JOIN `Post` AS t2 ON t1.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t1.`DateTime` < @DateTime AND t3.`Name` = @UserName LIMIT @Take) AS t0 INNER JOIN `Blog` AS t1 ON t0.`Id` = t1.`CategoryId`; SELECT t1.`Id`, t2.`Id`, t2.`BlogId`, t2.`UserId`, t2.`DateTime`, t2.`Content`, t3.`Id`, t3.`Name`, t3.`Email`, t3.`AccountId` FROM (SELECT DISTINCT t0.`Id` FROM `Category` AS t0 INNER JOIN `Blog` AS t1 ON t0.`Id` = t1.`CategoryId` INNER JOIN `Post` AS t2 ON t1.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t1.`DateTime` < @DateTime AND t3.`Name` = @UserName LIMIT @Take) AS t0 INNER JOIN `Blog` AS t1 ON t0.`Id` = t1.`CategoryId` INNER JOIN `Post` AS t2 ON t1.`Id` = t2.`BlogId` LEFT JOIN `User` AS t3 ON t2.`UserId` = t3.`Id`", actual);
    }
    ...
    {
        var query = Lt.Query<Post>().Include<User>(new[] { "User", "Blogs", "User" }).Where(_ => _.User!.Blogs.Any(_ => _.User.Name == Lt.Arg<string>("UserName"))).OrderBy(_ => _.Id).Skip("Skip").Take("Take").ToImmutable();
        var actual = _inst.CreateSelectSql(query);

        Assert.Equal("SELECT DISTINCT t0.`Id`, t0.`BlogId`, t0.`UserId`, t0.`DateTime`, t0.`Content`, t1.`Id`, t1.`Name`, t1.`Email`, t1.`AccountId` FROM `Post` AS t0 LEFT JOIN `User` AS t1 ON t0.`UserId` = t1.`Id` INNER JOIN `Blog` AS t2 ON t1.`Id` = t2.`UserId` INNER JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t3.`Name` = @UserName ORDER BY t0.`Id` LIMIT @Skip, @Take; SELECT t1.`Id`, t2.`Id`, t2.`Title`, t2.`CategoryId`, t2.`UserId`, t2.`DateTime`, t2.`Content`, t3.`Id`, t3.`Name`, t3.`Email`, t3.`AccountId` FROM (SELECT DISTINCT t1.`Id`, t0.`Id` AS _sort1 FROM `Post` AS t0 LEFT JOIN `User` AS t1 ON t0.`UserId` = t1.`Id` INNER JOIN `Blog` AS t2 ON t1.`Id` = t2.`UserId` INNER JOIN `User` AS t3 ON t2.`UserId` = t3.`Id` WHERE t3.`Name` = @UserName ORDER BY t0.`Id` LIMIT @Skip, @Take) AS t1 INNER JOIN `Blog` AS t2 ON t1.`Id` = t2.`UserId` INNER JOIN `User` AS t3 ON t2.`UserId` = t3.`Id`", actual);
    }
